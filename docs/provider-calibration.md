...
| LongCat billing | Token resource package and API pay-as-you-go balance | Watchlist | Frontend bundle exposes `token-packs/summary` and `api-usage/summary` dashboard billing endpoints, and unauthenticated replay returns login-required responses. Need a saved LongCat web-login authorization for live acceptance against a real account. | Keep LongCat as one provider and show Token Pack plus pay-as-you-go as two account-level billing meters. API keys remain companion copy-only credentials. |
...
| LongCat billing endpoints | Frontend bundle observation 2026-07-08; unauthenticated replay returned login-required bodies. | `POST https://longcat.chat/api/pay/quota/metering/token-packs/summary` exposes token-package fields such as `currentLot.remainingToken`, `currentLot.totalToken`, `currentLot.consumedToken`, `currentLot.expireTime`, and `otherLots[]`. `POST https://longcat.chat/api/pay/quota/metering/api-usage/summary` exposes pay-as-you-go fields such as `paygoBalance.primary.currency`, `paygoBalance.primary.amount`, `paygoStatus`, and recharge metadata. These endpoints require web login authorization; a LongCat API key alone is not sufficient for quota monitoring. |
...
| LongCat | Parser fixture only | Token Pack and pay-as-you-go dashboard endpoint field names observed from frontend bundle; live acceptance waits for a saved LongCat web-login authorization. |
