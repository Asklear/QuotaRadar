...
| AnySearch billing overview | Browser/frontend observation and authenticated account evidence 2026-08-01. | The `www.anysearch.com` console stores `search-template-auth-state`; refresh is `/api/ssuser/auth/refresh`, and billing overview is `/api/api/user/billing/overview`. The response exposes tier, used, remaining, total, reset period, and optional exact reset. A later saved-session replay returned business code `40114` (revoked refresh token) inside HTTP 200; this is authentication expiry, not schema drift. |
...
Live acceptance snapshot: 2026-08-01 CST.
...
| AnySearch | Reauthentication required | Saved refresh token was explicitly revoked (`40114` inside HTTP 200). Quota Radar now reports authentication expiry instead of schema drift; the Chrome console was logged out, so no fresh quota was claimed. |
