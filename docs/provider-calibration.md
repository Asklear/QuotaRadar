...
| AnySearch billing overview | Browser/frontend observation and authenticated account evidence 2026-08-01. | The `www.anysearch.com` console still stores `search-template-auth-state`; refresh is `/api/ssuser/auth/refresh`, and billing overview resolves to `/api/api/user/billing/overview`. The response exposes tier, used, remaining, total, reset period, and optional exact reset. The embedded console remained usable at 503 used / 497 remaining even while proactive refresh returned business code `40114` (revoked refresh token) inside HTTP 200. Quota Radar must therefore try the access token first and refresh only after an actual usage 401. |
