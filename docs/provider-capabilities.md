...
| Claude Subscription | LLM | 网页登录授权 | 仅订阅层级可读 | 待确认 | 否 | `edge-api/bootstrap` / `api/bootstrap/current_user_access` | 已验证 `claude.ai` 登录态可读 `rate_limit_tier`、订阅层级和 Claude Code 授权状态，但尚未找到稳定的 5 小时/周/月剩余额度接口；先仅保存网页登录授权。 |
...
| 阿里云 coding plan | LLM | 网页登录授权 | 控制台订阅状态 API；已预留用量字段解析 | 订阅状态；用量字段出现时按 5 小时/周/月窗口 | 否 | `BroadScopeAspnGateway` / `aliclaw.coding-plan`；另见 `codingPlan.queryCodingPlanInstanceInfoV2` | Coding Plan 官方定位为固定月费、月度请求额度；当前账号无套餐，富信息接口在无套餐场景返回业务错误。代码已支持接口返回 5 小时/周/月请求次数字段时按讯飞同口径展示。业务调用 key 可保存但不用于额度监控。 |
...
阿里云 Coding Plan 和腾讯云 Coding Plan 的业务调用 API Key 可以保存和展示，但额度监控使用网页登录授权。阿里云 Coding Plan 当前只能安全确认订阅状态；如果账号没有套餐会显示“未发现订阅套餐”，如果订阅有效但控制台未暴露用量明细会显示“可用 · 额度未知”。如果阿里云接口返回 5 小时/周/月的请求次数窗口，Quota Radar 会按讯飞星火和腾讯云同口径显示剩余次数/总次数。
...
以下结论来自 2026-06-08 的真实浏览器登录态和本地 QuotaService 脱敏验证。`resetAt` 指当前额度窗口重置时间，`planEndsAt` 指套餐/订阅结束时间。
...
| Claude Subscription | 仅订阅层级可读 | 待确认 | 待确认 | `claude.ai` `edge-api/bootstrap`、`api/bootstrap/current_user_access` 和 Claude Code 设置页可读 `rate_limit_tier`、功能权限、授权 token 状态；未确认稳定 5 小时/周/月剩余额度接口。 |
...
| 阿里云 coding plan | 订阅状态可查；用量字段出现时可解析 | 否 | 有套餐时可查 | `BroadScopeAspnGateway` / `aliclaw.coding-plan` 当前账号无套餐；可返回 `hasCodingPlan`、`clawQuota` 或 `codingPlanInfo.endTime`。另观察到 `codingPlan.queryCodingPlanInstanceInfoV2`，但无套餐场景返回业务错误；代码已预留 `usageDetail/perFiveHour/perWeek/perMonth` 或扁平 `limit/usage/left` 字段解析。 |
