...
| Claude Subscription | 已实测 | 2026-06-21 16:53 CST | `/api/organizations`、`/usage`、`/subscription_details` 当前返回套餐名、5 小时/周窗口、reset 和订阅周期结束时间。 | 不展示月窗口；不混入 Anthropic API/prepaid credits。organization tier 字段暴露倍率时，Max 显示为 `Max 5x` / `Max 20x`。 | 如果 organization 或 usage 字段变化，提示重新校准，而不是直接判定登录失效。 |
| Codex Subscription | 已实测 | 2026-06-21 16:53 CST | `/api/auth/session`、`/backend-api/wham/usage`、`/backend-api/subscriptions?account_id=...`、`/backend-api/accounts/check/v4-2023-04-27` 当前返回套餐层级、5 小时/周窗口、reset 和订阅周期结束时间。 | 不展示月窗口；使用 ChatGPT session `account.id` 查询生命周期，`accounts/check` 区分 `Pro 5x` 和 `Pro 20x`，`wham/usage` 仍然只作为额度来源。 | 如果生命周期字段变化，将 usage 解析和生命周期解析分开处理，并对生命周期部分提示重新校准。 |
...
| Codex Subscription | LLM | 网页登录授权；可选 `OPENAI_API_KEY` 仅用于保存/复制 | ChatGPT Codex Cloud Usage API | 5 小时 / 周；未见月窗口 | 否 | `/api/auth/session` + `/backend-api/wham/usage` + `/backend-api/subscriptions?account_id=...` + `/backend-api/accounts/check/v4-2023-04-27` | 已接入 ChatGPT session access token + `/backend-api/wham/usage` 刷新，显示 5 小时/周窗口和 reset；使用 `/api/auth/session` 的 `account.id` 查询订阅生命周期，并用 `accounts/check` 在可识别时显示 `Pro 5x` 或 `Pro 20x`。API Key 不参与订阅额度查询。 |
