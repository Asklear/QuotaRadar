...
| Claude Subscription | LLM | Web login authorization | Subscription tier only | Pending | No | `edge-api/bootstrap` / `api/bootstrap/current_user_access` | Verified that `claude.ai` login state exposes `rate_limit_tier`, subscription tier, and Claude Code authorization state, but no stable five-hour/weekly/monthly remaining-quota endpoint is confirmed; web login authorization is stored only. |
...
| Aliyun Coding Plan | LLM | Web login authorization | Dashboard subscription-status API; usage-field parser reserved | Subscription state; 5-hour/weekly/monthly windows when usage fields are exposed | No | `BroadScopeAspnGateway` / `aliclaw.coding-plan`; also observed `codingPlan.queryCodingPlanInstanceInfoV2` | Official docs describe Coding Plan as fixed monthly fee with monthly request quota. The current account has no package, and the richer endpoint returns a business error in that state. Code now parses 5-hour/weekly/monthly request-count fields if a subscribed account exposes them. Business invocation keys can be stored but are not used for quota monitoring. |
...
Aliyun Coding Plan and Tencent Cloud Coding Plan business invocation keys can be stored and shown, but quota monitoring uses web login authorizations. Aliyun Coding Plan currently confirms subscription status safely; accounts without a package show "No subscribed plan", while active subscriptions show "Usable · quota unknown" when the dashboard does not expose usage details. If Aliyun later exposes 5-hour/weekly/monthly request-count windows, Quota Radar will render them with the same remaining/total count model used for XFYun Spark and Tencent Cloud.
...
The following conclusions come from real browser login-state checks and local QuotaService redacted checks on 2026-06-08. `resetAt` means the current quota-window reset time; `planEndsAt` means the package/subscription end time.
...
| Claude Subscription | Subscription tier only | Pending | Pending | `claude.ai` `edge-api/bootstrap`, `api/bootstrap/current_user_access`, and the Claude Code settings page expose `rate_limit_tier`, feature entitlements, and authorization-token state; no stable five-hour/weekly/monthly remaining-quota endpoint is confirmed. |
...
| Aliyun Coding Plan | Subscription state; usage fields parse when exposed | No | When subscribed | `BroadScopeAspnGateway` / `aliclaw.coding-plan` currently reports no package on this account; it can return `hasCodingPlan`, `clawQuota`, or `codingPlanInfo.endTime`. `codingPlan.queryCodingPlanInstanceInfoV2` was also observed, but returns a business error without a package. Code now parses `usageDetail/perFiveHour/perWeek/perMonth` or flat `limit/usage/left` fields if a subscribed response exposes them. |
