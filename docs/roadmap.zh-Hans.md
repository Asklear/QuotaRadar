...
- [ ] 把当前 Tauri 版本当成迁移轨道，而不是 preview 交付：
  ...
  - 复刻 Swift v0.3.9 的 quota history、本地通知、provider 校准、release QA、白牌 / no-updater 行为，以及状态栏信号跳转 provider / 账号的语义。
...
P6 的历史 / 趋势 / 提醒主线，以及 v0.3.9 的白牌构建和重新认证加固已经完成。下一步重点是降低 provider 接口漂移风险，并让发版验收更可复用。
