...
- [x] 每个 provider 在配置页展示基础凭据类型：
  ...
  - 网页登录授权: Querit、Claude 订阅、Anthropic Credits、Codex 订阅、Kimi 订阅、讯飞星火 coding plan、火山引擎 coding plan、OpenCode Go、阿里云 coding plan、腾讯云 coding plan。
...
- [x] 为需要网页登录授权但用户仍要管理业务 API Key 的 provider 增加配套 API Key 保存：
  - Querit、Claude 订阅、Anthropic Credits、Codex 订阅、Kimi 订阅、讯飞星火 coding plan、火山引擎 coding plan、OpenCode Go、阿里云 coding plan、腾讯云 coding plan。
...
- [ ] 把当前 Tauri 版本当成迁移轨道，而不是 preview 交付：
  ...
  - 复刻 Swift v0.4.0 的 quota history、本地通知、provider 校准、release QA、白牌 / no-updater 行为，以及状态栏信号跳转 provider / 账号的语义。
...
P6 的历史 / 趋势 / 提醒主线，以及 v0.4.0 的 Codex 重置有效期和账号操作打磨已经完成。下一步重点是降低 provider 接口漂移风险，并让发版验收更可复用。
