...
- [ ] 从开发目录外验证 release bundle，优先使用 `/Applications/Quota Radar.app` 或生成的 DMG。
- [ ] 使用当前本地凭据 smoke refresh 火山引擎、Claude、Codex 和讯飞星火。
- [ ] 将功能分支合并回 `main`，在 `main` 上重新运行行为测试和视觉 QA，再打 release tag。
...
1. [x] 做一张 provider capability matrix。
   - 文件建议：新增 `docs/providers.md` / `docs/providers.zh-Hans.md`。
