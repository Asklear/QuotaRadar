...
impl KimiSubscriptionProvider {
    pub fn check_oauth_usage_fixture(
        ...
        self.check_response_fixture(credential, KIMI_QUOTA_UNAVAILABLE_FIXTURE, Some(KIMI_OAUTH_USAGE_FIXTURE))
    }
...
impl ProviderClient for KimiSubscriptionProvider {
    ...
    fn check_fixture_quota(
        ...
        self.check_response_fixture(credential, KIMI_SUBSCRIPTION_FIXTURE, Some(KIMI_USAGE_FIXTURE))
    }
}
...
fn parse_kimi_subscription_usage(
    ...
    let plan_ends_at = kimi_plan_end_date(&subscription)
        .or_else(|| usage.as_ref().and_then(kimi_plan_end_date));
    ...
    if let Some(window) = kimi_subscription_balance_window(&subscription, plan_ends_at.as_deref())
        .or_else(|| usage.as_ref().and_then(|usage| kimi_subscription_balance_window(usage, plan_ends_at.as_deref())))
    {
        windows.push(window);
    }
...
fn kimi_usage_windows(value: &Value) -> Vec<QuotaWindow> {
    ...
    if let Some(usages) = value.get("usages").and_then(Value::as_array) {
        let selected = usages
            .iter()
            .find(|usage| string_at(usage, "scope").map(|scope| scope.eq_ignore_ascii_case("FEATURE_CODING")).unwrap_or(false))
...
fn kimi_usage_detail_window(name: &str, source: &Value) -> Option<QuotaWindow> {
    ...
    let remaining = first_number(source, &["remaining", "remain", "left", "amountLeft", "amount_left"])
        .or_else(|| first_number(source, &["used", "usage", "amountUsed"]).map(|used| (limit - used).max(0.0)))?;
    let safe_remaining = remaining.max(0.0).min(limit);

    Some(QuotaWindow {
        name: name.to_string(),
        percent_remaining: Some(round_percent(safe_remaining / limit * 100.0)),
        remaining_text: Some(format!("{} / {}", compact_number(safe_remaining), compact_number(limit))),
        reset_at: first_string(source, &["resetTime", "resetAt", "reset_time", "reset_at"]),
    })
}

fn kimi_subscription_balance_window(value: &Value, plan_ends_at: Option<&str>) -> Option<QuotaWindow> {
    let balance = value
        .get("subscription_balance")
        .or_else(|| value.get("subscriptionBalance"))
        .or_else(|| value.get("creditBalance"))
        .or_else(|| {
            value.get("balances").and_then(Value::as_array).and_then(|balances| {
                balances
                    .iter()
                    .find(|balance| {
    ...
    let amount_left = first_number(balance, &["amount_left", "amountLeft", "left", "remaining", "remain"]);
    ...
    let (remaining_percent, remaining_text) = if let Some(amount) = amount.filter(|amount| *amount > 0.0) {
        ...
        (
            ...
            Some(format!("{} / {}", compact_number(remaining), compact_number(amount))),
        )
    } else if let Some(used_ratio) = used_ratio {
        (round_percent((100.0 - normalized_used_percent(used_ratio)).max(0.0)), None)
    } else {
        return None;
    };
...
fn kimi_plan_end_date(value: &Value) -> Option<String> {
    ...
    .or_else(|| {
        value.get("balances")
            ...
            .and_then(|balances| balances.iter().find_map(|balance| first_string(balance, &["expireTime", "expire_time"])))
    })
}
